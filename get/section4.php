...
<?php
				if (is_user_logged_in()) {
					...
					if ('0' != $categoryOnOff) { ?>
				<h3><span class="text-white badge bg-danger"> 6 no category </span></h3>
				<?php }
				} ?>
				<!-- category no on off end -->

				<?php
				$catId = get_theme_mod('rjs_category_dropdown_6'); // unique id of the framework
				if (empty($catId)) {
					$catId = 1; //changeable
				}
				query_posts(array(
				...
				<div class="row">
					<div class="col-12">
						<p class="text-uppercase bg-cat8 py-1 px-2 mb-2">
				...
				<div class="row mb-3 mb-md-0">
					<div class="col-md-8">
						<div id="entertainment" class="carousel slide mb-3" data-ride="carousel" data-interval="5000">
					            ...
					            while(have_posts()):the_post(); $active++ ?>
					                ...
					                <div class="carousel-inner">
					                  <div class="imageBox position-relative">
					                   ...
					                   <a class="carousel-control-prev" href="#entertainment" role="button" data-slide="prev">
					                      <span class="carousel-control-prev-icon" aria-hidden="true"></span>
					                      <span class="sr-only">Previous</span>
					                    </a>
					                    <a class="carousel-control-next" href="#entertainment" role="button" data-slide="next">
...
<?php
				if (is_user_logged_in()) {
					$categoryOnOff = get_theme_mod('npa_category_switcher_id');
					if ('0' != $categoryOnOff) { ?>
				<h3><span class="text-white badge bg-danger"> 7 no category </span></h3>
				<?php }
				} ?>
				<!-- category no on off end -->

				<?php
				$catId = get_theme_mod('rjs_category_dropdown_7'); // unique id of the framework
				if (empty($catId)) {
					$catId = 1; //changeable
				}
				query_posts(array(
				...
				<div class="row">
					<div class="col-12">
						<p class="text-uppercase bg-cat9 py-1 px-2 mb-2">
							<a class="text-white catTitleFontsize" href="<?php echo get_category_link($catId); ?>"> <?php echo get_cat_name($catId); ?> </a>
						</p>
					</div>
				</div>
...
<?php
				if (is_user_logged_in()) {
					$categoryOnOff = get_theme_mod('npa_category_switcher_id');
					if ('0' != $categoryOnOff) { ?>
				<h3><span class="text-white badge bg-danger"> 8 no category </span></h3>
				<?php }
				} ?>
				<!-- category no on off end -->

				<?php
				$catId = get_theme_mod('rjs_category_dropdown_8'); // unique id of the framework
				if (empty($catId)) {
					$catId = 1; //changeable
				}
				query_posts(array(
			...
			<div class="col-md-12">
				...
				<div class="row">
					...
					<?php while (have_posts()) : the_post(); ?>
						<div class="col-md-4 mb-3 col-6 poff">
